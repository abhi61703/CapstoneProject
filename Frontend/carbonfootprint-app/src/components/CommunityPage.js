...
const CommunityPage = () => {
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get('http://localhost:8082/community', {
  ...
  const handlePostSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8082/community/upload', formData, {
        headers: {
  ...
  const handleLike = async (postId) => {
    ...
    try {
      const response = await axios.post(`http://localhost:8082/community/${postId}/like`, {}, {
        headers: { 'Authorization': `Bearer ${token}` },
      });
  ...
  const handleCommentSubmit = async (postId, e) => {
    ...
    try {
      const response = await axios.post(`http://localhost:8082/community/${postId}/comment`, null, {
          ...
          <div className="bg-white p-4 rounded shadow-lg max-w-4xl mx-auto">
            {posts.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {posts.map((post) => (
                  <div key={post.postId} className="border p-4 rounded shadow-lg bg-white">
